...
async function getFreshStreamUrl() {
  ...
  console.log('Iniciando Chromium en Render...');
  ...
  try {
    ...
    // Escuchar todas las peticiones, incluyendo subframes e iFrames
    page.on('request', req => {
      const url = req.url();
      if (url.includes('.m3u8')) {
        console.log('Detectado archivo m3u8:', url);
    ...
    // Intentar simular clic en el reproductor si existe para forzar la reproducción
    try {
      await page.click('video, .vjs-big-play-button, iframe');
    } catch (e) {
      // Si no encuentra el botón de play, continúa esperando
    }
  ...
  if (streamUrl) {
    ...
    throw new Error('No se pudo interceptar la señal tras simular reproduccion');
  }
}
...
app.get('/americatv.m3u8', async (req, res) => {
  try {
    ...
    res.status(500).send('Error obteniendo la señal de América TV: ' + error.message);
  }
});
