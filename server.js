...
async function getFreshStreamUrl() {
  ...
  try {
    ...
    // Interceptar las peticiones de red para capturar la URL .m3u8 de Mediastream
    page.on('request', req => {
      const url = req.url();
      if (url.includes('.m3u8') && (url.includes('mdstrm') || url.includes('mediastream'))) {
        if (!streamUrl) {
          streamUrl = url;
          console.log('¡URL de stream capturada con éxito!');
        }
      }
    });
  ...
  if (streamUrl) {
    ...
    throw new Error('No se pudo interceptar la señal en vivo de América TV');
  }
}
...
app.listen(PORT, () => {
  console.log(`Proxy Chromium listo en puerto ${PORT}`);
});